var barcode = (function () {
  ...
  function init() {
    ...
    // Inicializar elementos con validaciones
    ...
    if (!elements.video || !elements.canvas || !elements.canvasg) {
      console.error(
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
		  ...
		  elements.video.addEventListener("canplay", function () {
			...
			// Dimensiones de la zona activa
			dimensions.start = dimensions.width * 0.15; // 15% del ancho
			dimensions.end = dimensions.width * 0.85;  // 85% del ancho
			dimensions.activeTop = dimensions.height * 0.35; // 35% de la altura
			dimensions.activeBottom = dimensions.height * 0.65; // 65% de la altura
			dimensions.activeMiddle = Math.round(dimensions.activeTop + (dimensions.activeBottom - dimensions.activeTop) / 2);
			...
			// Dibuja la línea roja centrada en el rectángulo activo
			elements.ctxg.clearRect(0, 0, dimensions.width, dimensions.height); // Limpia el canva
			drawGraphics();
		  
			// Captura de imágenes a intervalos
  ...
  function drawGraphics() {
	...
	elements.ctxg.clearRect(0, 0, dimensions.width, dimensions.height);
  
	// Dibuja el rectángulo verde
	elements.ctxg.strokeStyle = "#f0eded";
	elements.ctxg.lineWidth = 3;
	elements.ctxg.strokeRect(
	  dimensions.start,                       // Inicio horizontal del rectángulo
	  dimensions.activeTop,                  // Inicio vertical del rectángulo
	  dimensions.end - dimensions.start,     // Ancho del rectángulo
	  dimensions.activeBottom - dimensions.activeTop // Altura del rectángulo
	);
  
	// Dibuja la línea roja centrada
	elements.ctxg.strokeStyle = "#ff0000";
	elements.ctxg.lineWidth = 3;
	elements.ctxg.beginPath();
	elements.ctxg.moveTo(dimensions.start, dimensions.activeMiddle); // Desde el inicio del rectángulo
	elements.ctxg.lineTo(dimensions.end, dimensions.activeMiddle);   // Hasta el final del rectángulo
	elements.ctxg.stroke();
  
	// Mensajes de depuración visual (opcional)
	console.log("Dibujando línea roja en:", dimensions.activeMiddle);
  }
